        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            for c in det[:, -1].unique():
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    namelabel = f'{names[int(cls)]}'
                    # self.plot_one_box(xyxy, image, label=label, line_thickness=3)
                    if namelabel == 'mask':
                        self.plot_one_box(xyxy, image, label=namelabel, color=[1, 225, 167], line_thickness=3)
                    elif namelabel == 'face':
                        self.plot_one_box(xyxy, image, label=namelabel, color=[43, 48, 199], line_thickness=3)
        # 后处理
        cv2.imwrite('./result.jpg', image)
        return ((x1, y1, x2, y2),namelabel)
